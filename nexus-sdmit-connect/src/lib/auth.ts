...
class AuthService {
  ...
  parseUSN(usn: string) {
    // USN format: 4SU22AD023
    const year = parseInt(usn.substring(4, 6)) // 22
    const branchCode = usn.substring(6, 8) // AD
  ...
  register(name: string, email: string, password: string, usn: string): Promise<boolean> {
    return new Promise((resolve) => {
      setTimeout(() => {
